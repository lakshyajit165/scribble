...
export class DashboardComponent implements OnInit {
  ...
  loadScribbles(
    ...
    this._notesService
      .searchNotes(searchText, fromDate, toDate, page, size)
      .subscribe({
  ...
  loadScribblesByDebounce(): void {
    this.loading = true;
    this.debounceSubcription = this.searchScribbleFormGroup.valueChanges
      .pipe(
        debounceTime(500),
        distinctUntilChanged(),
        switchMap((fields) =>
          this._notesService.searchNotes(
            ...
            this.selectedFromDate
              ? moment(this.selectedFromDate).format('YYYY-MM-DD').toString()
              : '',
            this.selectedToDate
              ? moment(this.selectedToDate).format('YYYY-MM-DD').toString()
              : '',
            this.page,
            this.size
          )
...
In an Observable Execution, zero to infinite Next notifications may be delivered.
 ...
 Hence the recursive call to subscribe to value changes again is there.
  ...
  openDialog(): void {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      ...
      if (result) {
        ...
        const formattedFromDate = fromDatePicked
          ? moment(fromDatePicked).format('YYYY-MM-DD').toString()
          : '';
        const formattedToDate = toDatePicked
          ? moment(toDatePicked).format('YYYY-MM-DD').toString()
          : '';
  ...
  getNumberOfFiltersApplied(): number {
    if (this.selectedFromDate && this.selectedToDate) return 2;
