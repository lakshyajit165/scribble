...
export class SignupComponent implements OnInit {
  ...
  hideVerificationCode: boolean = true;
  ...
  signUpUser(stepper: MatStepper): void {
    ...
    if (this.emailFormGroup.valid) {
      this.signUpAndForgotPasswordPayload.email =
  ...
  confirmPassword(): void {
    ...
    if (this.confirmPasswordFormGroup.valid) {
      this.confirmPasswordPayload.email =
        this.emailFormGroup.get('email')?.value ?? '';
      this.confirmPasswordPayload.password =
        this.confirmPasswordFormGroup.get('password')?.value ?? '';
      this.confirmPasswordPayload.verification_code =
        this.confirmPasswordFormGroup.get('verificationCode')?.value ?? '';
      this._authService.confirmPassword(this.confirmPasswordPayload).subscribe({
        next: (data: IGenericResponse) => {
          this.confirmPasswordLoading = false;
          this._snackBarService.showSnackBar(
            data.message || 'SignUp successful, logging in!',
            3000,
            'check_circle_outline'
          );
          /**
